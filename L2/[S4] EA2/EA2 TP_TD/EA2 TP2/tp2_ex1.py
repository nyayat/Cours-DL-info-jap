...
def fibo_1_adds(n) :
    if n <= 0 : return 0, 0
    if n <= 2 : return 1, 0
    res=fibo_1_adds(n-1)[0]+fibo_1_adds(n-2)[0]
    ops=fibo_1_adds(n-1)[1]+fibo_1_adds(n-2)[1]+1
    return res, ops            
...
# Le nombre d'addition est le même pour fibo_2 et fibo_3, alors qu'en réalité,
# fibo_3 est plus performant que fibo_2.
# Donc les mesures que nous avons ne reflètent pas les temps d'exécution.
...
def fibo_1_bits(n) :
    if n <= 0 : return 0, 0
    if n <= 2 : return 1, 0
    res=fibo_1_bits(n-1)[0]+fibo_1_bits(n-2)[0]
    ops=fibo_1_bits(n-1)[1]+fibo_1_bits(n-2)[1]+nbOfBits(res)
    return res, ops
