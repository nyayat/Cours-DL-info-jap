...
def additionV(nb1, nb2) :
  # A REMPLIR
    ...
    op=0
    if(len(nb1)<len(nb2)):
        for i in range(len(nb2)-len(nb1)):
            nb1+=[0]
    elif(len(nb1)>len(nb2)):
        for i in range(len(nb1)-len(nb2)):
            nb2+=[0]
    for(c1, c2) in zip (nb1, nb2):
        tmp=c1+c2+retenue
        retenue=tmp//10
        res.append(tmp%10)
        op+=4
...
#On n'a pas plus d'opérations élémentaires si les nombres sont de tailles
#différentes, car on a juste agrandi le tableau du plus petit nombre
#(avec des 0, ce qui n'impacte pas l'addition), pour que les deux
#tableaux aient autant de cases chacun.
...
def testDataAddEq() :
  return [ [ [[0, 1], [1, 1]], [[1, 2], 8] ],
       ...
       [[[7,5,6,6,6,6], [2,9,8,7,1,5]], [[9,4,5,4,8,1,1], 24]]
	 ]
